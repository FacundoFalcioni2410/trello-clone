...
export default function BoardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleSaveCard() {
    ...
    // Optimistic update — replace the card in-place so it doesn't jump
    setBoard((prev) => {
      if (!prev) return prev;
      return {
        ...prev,
        lists: prev.lists.map((l) => ({
          ...l,
          cards: l.cards.map((c) => (c.id === selectedCard.card.id ? updatedCard : c)),
        })),
      };
    });
    ...
    <div className="flex h-screen flex-col" style={{ backgroundColor: bg }}>
      ...
      {selectedCard && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm" onClick={() => setSelectedCard(null)}>
          <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl dark:bg-zinc-900" onClick={(e) => e.stopPropagation()}>
